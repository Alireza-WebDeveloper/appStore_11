import { useSignIn } from '@/app/lib/hooks/auth';
import { Button } from '@nextui-org/react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { InputController } from '@/app/lib/design/common/input-controller';
import useForgotPasswordStore from '@/app/lib/store/forgot-password';
import {
  resetPasswordSchema,
  ResetPasswordSchema,
} from '@/app/lib/schema/forgot-password';
import { useResetPassword } from '@/app/lib/hooks/auth/forgot-password';
import CountdownTimer from '@/app/(auth)/auth/components/count-down-timer';

const ResetPassword = () => {
  const { email, code, expiresAt } = useForgotPasswordStore();

  const { mutate: resetPasswordMutate, isLoading } = useResetPassword();

  const onSubmit = (data: ResetPasswordSchema) => {
    const { newPassword } = data;
    resetPasswordMutate({ newPassword, email, code });
  };

  const { handleSubmit, control, trigger } = useForm<ResetPasswordSchema>({
    defaultValues: {
      newPassword: '',
    },
    resolver: zodResolver(resetPasswordSchema),
  });

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      <InputController
        trigger={trigger}
        control={control}
        name="newPassword"
        label="رمز عبور جدید"
        type="password"
      />
      <Button
        aria-label="sign-in"
        isLoading={isLoading}
        type="submit"
        color="success"
        size="lg"
      >
        تغییر رمز عبور
      </Button>{' '}
      <CountdownTimer expiresAt={expiresAt} />
    </form>
  );
};

export default ResetPassword;
